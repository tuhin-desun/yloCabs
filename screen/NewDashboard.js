...
import { getAvailableCars, makeBookRequest, getUserProfile, cancelBooking, cancelSearchBooking, getAdvertisementImages, getPopUp } from '../services/APIServices';
import AppContext from "../context/AppContext";
import firebase from "../config/firebase";
import { tempBooking } from "../utils/helper";
import { generateBookingId, getFormattedDate, generateOTP } from "../utils/Util";
...
export default class Dashboard extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.state.currentDriver != prevState.currentDriver) {
            if (this.state.currentDriver != null) {
                this.firebaseTempBookinfRef = firebase.database().ref('TempBooking/' + this.state.currentDriver);
    ...
    getPopUpAdvertisementImages = () => {
        getPopUp({ is_popup: 1 }).then((response) => {
            ...
            if (value != 'null' && value != 'undefined') {
                ...
                if (value.status == 'ACCEPTED') {
                    ...
                    this.setState({
                        ...
                        this.props.navigation.navigate('ReachedScreen', { 'curBooking': value });
                    })
                ...
                if (value.status == 'PAYMENT_PENDING') {
                    this.props.navigation.navigate('PaymentDetails', { 'curBooking': value });
                }
            }
        });
    }
                        ...
                        if (this.state.activeKeyTrip == 'daily') {
                            ...
                            if (distance < 40) {
                               ...
                               */
                                ...
                                if (currentTime >= item.phourfrom || currentTime <= item.phourto) {
                            ...
                            item.price = Math.ceil(Number(baseFare) + (chargeAbleDistance * perKmCharge))

                            console.log("Car******", baseFare, chargeAbleDistance, perKmCharge, item, item.price);
                            ...
                            if (item.distance == 0) {
                                ...
                                chargeAbleDistance = Number(this.state.distance) - Number(item.distance);
                            }
                            console.log("Round Trip   ", item.name, this.state.distance * item.outstation_round_trip, this.state.distance, item.outstation_round_trip, diffDays)
                            // if (diffDays > 0) {
    ...
    getAllCars = () => {
        Promise.all([getAvailableCars({ location: this.state.location }), getAdvertisementImages()])
            .then((response) => {
    ...
    getCars = async (reqObj) => {
        ...
        getAvailableCars({ 'service_type': serviceTypeID, 'location': this.state.location })
            .then((response) => {
                let closestDriver = [];
                let selectedDriver = [];
                let originalArr = [];
                if (this.state.previouslyCalledCars.length > 0) {
                    ...
                    if (original.length > 0) {
                        closestDriver = getCloseDriver(this.state.location, original, reqObj);
                    ...
                    if (response[0].provider_data.length > 0) {
                        closestDriver = getCloseDriver(this.state.location, response[0].provider_data, reqObj);
                        selectedDriver.push(closestDriver.id);
                        this.setState({
                            previouslyCalledCars: selectedDriver
                        })
    ...
    initiateBooking = (reqObj, driver_id) => {
        ...
        makeBookRequest(reqObj).then((response) => {
    ...
    bookingPreview = async () => {
        ...
        let fare = Math.ceil(Number(baseFare) + Number((travelDistance * perKmCharge)));
        let dueAdd = 0;
    ...
    bookingRoundTrip = async () => {
        ...
        let fare = Math.ceil(Number(baseFare) + Number(((travelDistance * outstation_round_trip) * diffDays)));
    ...
    notifyDriver = async () => {
        ...
        const { serviceTypeID, carName, tripdata, estimate, coordinates, currentAddress, destinationAddress, distance, duration, base_fare } = this.state;
        ...
        let reqObj = { serviceTypeID, carName, coordinates, tripdata, estimate, currentAddress, destinationAddress, user_id, distance, duration, base_fare, bookingID, status, OTP, user_name, user_mobile, user_avatar, currentTimeSeconds }
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.mapcontainer}>
                    {this.state.location !== null ? (
                        ...
                        >
                            ...
                            {this.state.freeCars ? this.state.freeCars.map((item, index) => {
                                return (
                                    <Marker.Animated
                                        coordinate={{ latitude: item.location ? Number(item.location.latitude) : 0.00, longitude: item.location ? Number(item.location.longitude) : 0.00 }}
                                        key={index}
                                    >
                                        <Image
                                            key={index}
                                            source={item.carImage ? { uri: item.carImage } : require("../assets/microBlackCar.png")}
                    ...
                    {this.state.placeSelectionType == 'pickup' ?
                        (<View pointerEvents="none" style={styles.mapFloatingPinView}>
                            <Image pointerEvents="none" style={[styles.mapFloatingPin, { marginBottom: 40 }]} resizeMode="contain" source={require("../assets/green_marker_1.png")} />
                        </View>)
                        :
                        (<View pointerEvents="none" style={styles.mapFloatingPinView}>
                            <Image pointerEvents="none" style={[styles.mapFloatingPin, { marginBottom: 40 }]} resizeMode="contain" source={require('../assets/red_marker.png')} />
                        </View>)
                    }
                    <View style={styles.locationButtonView}>
                        <TouchableOpacity onPress={this.getCurrentPosition} style={styles.locateButtonStyle}>
                            <Icon
                                name='gps-fixed'
                ...
                <BottomSheet
                    ...
                    footerComponent={this.state.activeKeyTrip == 'daily' ? this.renderFirstFooter : this.renderSecondFooter}
                >
                    <>
                        {this.state.distance > 50 ? (
                            <View style={styles.tripTabContainer}>
                                <TouchableOpacity
                                    activeOpacity={0.7}
                                    style={{ borderRadius: 5, width: '48%', justifyContent: 'center', alignItems: 'center', backgroundColor: this.state.activeKeyTrip == 'daily' ? colors.primary : colors.lightGrey, paddingVertical: 15 }}
                                    onPress={this.handleActiveTripKeyChange.bind(this, 'daily')}
                                >
                                    <Text style={{ color: this.state.activeKeyTrip == 'daily' ? colors.WHITE : colors.grey }}>Daily</Text>
                                </TouchableOpacity>

                                <TouchableOpacity
                                    activeOpacity={0.7}
                                    style={{ borderRadius: 5, width: '48%', justifyContent: 'center', alignItems: 'center', backgroundColor: this.state.activeKeyTrip == 'daily' ? colors.lightGrey : colors.primary, paddingVertical: 15 }}
                                    onPress={this.handleActiveTripKeyChange.bind(this, 'round')}
                                >
                                    <Text style={{ color: this.state.activeKeyTrip == 'daily' ? colors.grey : colors.WHITE }}>Round Trip</Text>
                                </TouchableOpacity>
                            </View>
                        ) : null}
                            ...
                            <>
                                {this.state.activeKeyTrip == 'daily' ? (
                                    ...
                                    <View style={{ width: '100%' }}>
                                        <View style={{ width: '98%', justifyContent: 'center', alignSelf: 'center', marginTop: 10 }}>
                                            <Text style={styles.modalText}>Return Date</Text>
                                            <View
                                                style={[
                                                    styles.formField,
                                                    this.state.returnDateValidation ? styles.errorField : null,
                                                ]}
                                            >
                                                <DatePicker
                                ...
                                <View style={{ flex: 1, marginHorizontal: 10, marginTop: 10 }}>
                                    {this.state.advertisementData?.length > 0 ? this.state.advertisementData.map((item, index) => {
                                        return (
                                            <TouchableOpacity
                                                activeOpacity={1}
                                            ...
                                            >
                                                <Image
                                                    ...
                                                    style={{ height: 200, width: "100%", borderRadius: 10, marginBottom: 10 }}
                                                />
                                            </TouchableOpacity>
                                        )
                                    }) : null}
                ...
                >
                    <View style={{ flex: 1, backgroundColor: "rgba(22,22,22,0.8)", justifyContent: 'center', alignItems: 'center' }}>
                        <View style={{ width: '80%', backgroundColor: Colors.WHITE, borderRadius: 10, justifyContent: 'center', alignItems: 'center', flex: 1, maxHeight: 310, marginTop: 15 }}>
                            <Image source={require('../assets/lodingDriver.gif')} resizeMode={'contain'} style={{ width: 160, height: 160, marginTop: 15 }} />
                            <View><Text style={{ color: Colors.BLUE.default.primary, fontSize: 16, marginTop: 12, marginBottom: 10 }}>{"Please wait finding driver for you"}</Text></View>
                            <View>
                                <Button
                                    title={"close"}
                                    loading={false}
                                    ...
                                    buttonStyle={{ borderRadius: 5, backgroundColor: Colors.primary }}
                                    containerStyle={{
                                        height: 50,
                                        width: 100,
                                    }}
                                ...
                                <View style={styles.modalBody}>
                                    <>
                                        <TouchableOpacity
                                            ...
                                            style={{ alignSelf: 'flex-end', marginBottom: -45, zIndex: 99, marginRight: 5 }}>
                                            <AntDesign name="closecircle" size={40} style={{ backgroundColor: 'black', borderRadius: 50 }} color={Colors.primary} />
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            activeOpacity={1}
                                            onPress={this.linkHandler.bind(this, this.state.advtModalURL)}
                                        >
                                            <Image source={{ uri: this.state.advtModalImgURL }} style={{ height: windowHeight / 3.5, width: '100%', borderRadius: 10 }} />
                                        </TouchableOpacity>
                                    </>
                                </View>
                    ...
                    >
                        <View>
                            <View style={[styles.modalView, { backgroundColor: Colors.primary }]}>
                                <View style={[styles.modalHead, { backgroundColor: Colors.primary, borderBottomWidth: 1, borderBottomColor: '#959595' }]}>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', height: windowHeight / 14 }}>
                                    <TouchableOpacity
                                        onPress={() => {
                                            this.setState({
                                                outstationModalVisible: !this.state.outstationModalVisible
                                        ...
                                        style={{ justifyContent: 'center', alignItems: 'center', width: '50%', borderTopWidth: 1, borderTopColor: "#959595", borderRightWidth: 1, borderRightColor: '#959595' }}
                                    ><Text style={{ color: Colors.grey, fontWeight: 'bold' }}>CANCEL</Text></TouchableOpacity>
                                    <TouchableOpacity
                                        onPress={this.bookingPreview2}
                                        style={{ justifyContent: 'center', alignItems: 'center', width: '50%', borderTopWidth: 1, borderTopColor: "#959595" }}
                                    ><Text style={{ fontWeight: 'bold' }}>DONE</Text></TouchableOpacity>
                                </View>
