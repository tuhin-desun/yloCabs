...
export default function PaymentDetails(props) {
  console.log("Pay booking", props.route.params.curBooking);
  const context = useContext(AppContext);
  const booking = props.route.params.curBooking;
    ...
    <View style={styles.mainView}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollStyle}>
        ...
        <View style={{ flex: 1, flexDirection: 'column' }}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 10, paddingRight: 10 }}>
            ...
            <Text style={{ color: colors.BLACK, textAlign: 'left', lineHeight: 35, fontSize: 16 }}>{"₹ "}{booking && Math.round(booking?.estimate?.estimateFare)}{'.00'}</Text>
          </View>
      ...
      <View style={{ height: 110 }}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[styles.buttonWrapper, { flex: 1, backgroundColor: colors.primary }]}
            onPress={() => {
              doPayment('cash');
            }}>
